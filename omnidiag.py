...
This is the main CLI entry point that orchestrates:
  ...
  3. AI-powered root-cause analysis via MiMo LLM
...
MIMO_MAX_TOKENS = int(os.getenv("MIMO_MAX_TOKENS", "8192"))
...
def call_mimo_api(probe_output: str, knowledge_context: str, auto_fix: bool = False) -> dict:
    """
    Send diagnostic data to MiMo LLM for AI-powered analysis.
    ...
    """
    if not MIMO_API_KEY:
        print("\033[31m[ERROR] MIMO_API_KEY not set. Please configure .env file.\033[0m")
        print("  Get your API key at: https://xiaomimimo.com")
        sys.exit(1)
...
"""
    ...
    print(f"\033[35m[AI] Sending {len(user_prompt):,} chars to MiMo ({MIMO_MODEL})...\033[0m")
    ...
    try:
        ...
        print(f"\033[31m[ERROR] MiMo API call failed: {e}\033[0m")
        return {"error": str(e)}
...
def save_fix_script(diagnosis: dict, probe_name: str) -> Path | None:
    ...
    with open(fix_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"# Probe: {probe_name} | Model: {MIMO_MODEL}\n")
...
def cmd_diagnose(args):
    ...
    # Step 3: AI Analysis
    diagnosis = call_mimo_api(probe_output, knowledge, auto_fix=args.auto_fix)
